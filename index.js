...
// Store the original game body content on page load
const gameContent = document.body.innerHTML;

// Fetch and store about page content
...
fetch('About.html')
    ...
    .then(html => {
        ...
        aboutContent = doc.body.innerHTML;
    });

function showAbout() {
    if (game_started) return; // Don't allow navigation mid-game
    document.body.innerHTML = aboutContent;
    document.title = "About - Memory Game";
}
...
function showGame() {
    ...
    // Re-attach jQuery event listeners after DOM is replaced
    bindEvents();
}
